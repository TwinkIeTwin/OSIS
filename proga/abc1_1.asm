...
MONO_SEG        EQU     0b000   ; сегмент видеопамяти монохромного видеоадаптера
COLOR_SEG       EQU     0b800   ; сегмент видеопамяти цветного видеоадаптера
CRT_SIZE        EQU     4000    ; размер сегмента видеопамяти цветного видеоадаптера
MONO_SIZE       EQU     1000    ; размер сегмента видеопамяти монохромного видеоадаптера
CRT_LOW         EQU     8000    ; мл. байт физического адреса сегмента видеопамяти цветного видеоадаптера                                
MONO_LOW        EQU     0000    ; мл. байт физического адреса сегмента видеопамяти монохромного видеоадаптера
CRT_SEG         EQU     0Bh     ; ст. байт физического адреса сегмента видеопамяти
                                

; Селекторы, определённые в таблице GDT (значения берутся из глобальной таблицы дискрипторов)
...
PORT_6845       EQU     0063h   ; адрес области данных BIOS, где записано значение адреса порта контроллера 6845                             
...
; Определяем базовый адрес видеопамяти
        call    set_crt_base ; !Реализация процедуры находится ниже 
...
hello_msg db " Protected mode "
end_msg db " Real mode "
...
; Загружаем адрес выводимой строки и её длину
        ...
        mov     dh,40h  ; аттрибут отвечает за цвет
